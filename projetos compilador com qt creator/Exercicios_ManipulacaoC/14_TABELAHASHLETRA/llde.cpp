...
void LLDE::inserirOrdenado(Dados dado,QString nome){
    try{
        if(estaVazia()){
            inserirInicio(dado);
            return;
        ...
        for(int pular=0;pular<quantidadeElementos;pular++){
                QChar P1Letra=nome.at(1);
                char Ascii1Value=P1Letra.toLatin1();
                int ASCII=Ascii1Value;

                QChar PLetra=pAux->getDado().getNome().at(1);
                char AsciiValue = PLetra.toLatin1();
                int ValorAscii=AsciiValue;
                int contador=2;
                while(ValorAscii==ASCII){
                    P1Letra=nome.at(contador);
                    Ascii1Value=P1Letra.toLatin1();
                    ASCII=Ascii1Value;

                    PLetra=pAux->getDado().getNome().at(contador);
                    AsciiValue = PLetra.toLatin1();
                    ValorAscii=AsciiValue;
                    contador++;
